...
(() => {
  ...
  const BUILD_VERSION = '5.10.4';
  const BUILD_STAMP = 'BUILD ETHEREUM-KRAKEN-FIX · 2026-08-14 · 14:32';
    ...
    runSystemDiagnosis = async function() {
      ...
      try {
        ...
        showDiagnostic('Systemprüfung abgeschlossen', [
          ...
          'Aktive UI-Architektur: app.js + app-release-5104.js · ein konsolidierter Daten-/UI-Layer',
...
/* Depot-Cockpit 5.10.4 – DATA CORE V3
...
(() => {
  ...
  async function refresh590() {
    ...
    showDiagnostic('Datenkern 5.10.4 prüft alle Depotpositionen', [
    ...
    try {
      ...
      showDiagnostic(
        body.complete ? 'Datenkern 5.10.4: vollständige Versorgung' : 'Datenkern 5.10.4: Prüfung abgeschlossen',
        lines,
        body.complete ? 'success' : 'info'
      );
      toast(`${Object.keys(stored.items || {}).length}/${state.positions.length} Positionen verwertbar`);
    } catch (error) {
      showDiagnostic('Datenkern 5.10.4 fehlgeschlagen', [
...
(() => {
  ...
  const VERSION_595 = '5.10.4';
  const BUILD_595 = 'BUILD ETHEREUM-KRAKEN-FIX · 2026-08-14 · 14:32';
