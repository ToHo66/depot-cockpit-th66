...
const APP_VERSION='GOLDEN MASTER 5.1';
...
const state={positions:structuredClone(DEFAULT_POSITIONS),archive:[],transactions:[],data:{},apiDiagnostics:null,health:null,settings:{sbrokerReference:null,sbrokerReferenceUpdatedAt:null,trReference:null,trReferenceUpdatedAt:null,brokerPrices:{},brokerPositionValues:{},venuePriority:['Tradegate','gettex','Lang & Schwarz','Xetra','Stuttgart','Frankfurt']},updatedAt:null};
...
function renderDiagnostics(){
  ...
  box.innerHTML=`<div class="diagnostic-grid">
    ...
    <span>API-Key vorhanden<b>${state.apiDiagnostics?.apiKeyPresent===true||state.health?.eodhdConfigured===true?'Ja':state.apiDiagnostics?.apiKeyPresent===false||state.health?.eodhdConfigured===false?'Nein':'Noch nicht geprüft'}</b></span>
    <span>Requests letzte Sitzung<b>${state.apiDiagnostics?.requestCount??'–'}</b></span>
    <span>Erfolgreich / fehlgeschlagen<b>${state.apiDiagnostics?`${state.apiDiagnostics.successfulRequests} / ${state.apiDiagnostics.failedRequests}`:'–'}</b></span>
    <span>Letzte erfolgreiche Antwort<b>${state.apiDiagnostics?.lastSuccessfulAt?new Date(state.apiDiagnostics.lastSuccessfulAt).toLocaleString('de-DE'):'–'}</b></span>
    <span>Vercel Deployment-ID<b>${state.apiDiagnostics?.deploymentId||state.health?.deploymentId||'nicht verfügbar'}</b></span>
    <span>Vercel Umgebung<b>${state.apiDiagnostics?.environment||state.health?.environment||'–'}</b></span>
    <span>Serverpfad<b>${state.apiDiagnostics?.sourceLayer||'/api/market-data'}</b></span>
    <span>App-Version<b>${APP_VERSION}</b></span>
    <span>Alte Speicher auf dieser Adresse<b>${oldKeys.length?oldKeys.join(', '):'keine'}</b></span>
  </div>
  <div class="api-diagnostic-actions"><button id="runApiDiagnostic" class="primary-action">API-Diagnose starten</button></div>
  <div class="api-attempt-list">${Object.values(state.data).flatMap(d=>(d?.attempts||[]).map(a=>`<div class="api-attempt ${a.ok?'ok':'error'}"><b>${a.symbol||'–'} · ${a.venue||'–'}</b><span>HTTP ${a.httpStatus??'Netzwerk'} · ${a.responseTimeMs??'–'} ms</span><small>${a.endpoint||'–'}</small><small>${a.ok?'Antwort erfolgreich':(a.responsePreview||d.error||'Keine Antwort')}</small></div>`)).join('')||'<p class="muted">Noch keine Einzelabfragen protokolliert.</p>'}</div>
  <p class="diagnostic-note">Diagnose-Release: Es werden keine API-Schlüssel angezeigt. Die Kurs-, Cache-, Bewertungs- und Designlogik wurde nicht verändert.</p>`;
  const run=$('#runApiDiagnostic');if(run)run.onclick=refresh
}
...
load();loadMarketCache();wire();render();fetch('/api/health',{cache:'no-store'}).then(r=>r.json()).then(x=>{state.health=x;render();if(!x.eodhdConfigured){const b=$('#setupBanner');b.classList.remove('hidden');b.innerHTML='<b>Golden Master 5.1 ist aktiv.</b><br>Für EODHD-Marktdaten fehlt der geschützte Schlüssel in Vercel.'}else{refresh()}}).catch(()=>{});
