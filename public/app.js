...
const MARKET_CACHE='th66-professional-market-cache-v321';
...
function render(){
  ...
  renderMarketToday();renderOverviewPositions();renderPositions();renderAnalysis();renderBrokerComparison();renderManage();renderCourseManager();renderTransactions()
}
...
function renderBrokerComparison(){
  ...
  const rows=state.positions.map(p=>{const brokerVal=brokerPositionValue(p),appVal=appEodValue(p),usedVal=positionValue(p),diff=Number.isFinite(brokerVal)&&Number.isFinite(appVal)?appVal-brokerVal:null;return {p,brokerVal,appVal,usedVal,diff}});
  const known=rows.filter(x=>Number.isFinite(x.brokerVal));
  const brokerSum=known.length?known.reduce((a,x)=>a+x.brokerVal,0):null;
  const appSum=known.length?known.reduce((a,x)=>a+(Number.isFinite(x.appVal)?x.appVal:0),0):null;
  const summary=known.length?`<div class="comparison-summary"><span>Erfasste Positionen <b>${known.length}/${rows.length}</b></span><span>Broker-Summe <b>${eur(brokerSum)}</b></span><span>App-EOD-Summe <b>${eur(appSum)}</b></span><span>Abweichung <b class="${cls(appSum-brokerSum)}">${eur(appSum-brokerSum)}</b></span></div>`:'<p class="muted">Noch keine zeitgleichen Broker-Positionswerte erfasst.</p>';
  box.innerHTML=summary+rows.map(x=>`<div class="comparison-row"><div><b>${x.p.name}</b><small>${x.p.qty.toLocaleString('de-DE')} Stück · ${x.p.broker}</small></div><div class="comparison-values"><span>Broker <b>${eur(x.brokerVal)}</b></span><span>App EOD <b>${eur(x.appVal)}</b></span><span>Differenz <b class="${cls(x.diff)}">${eur(x.diff)}</b></span></div></div>`).join('');
}
...
load();loadMarketCache();wire();render();fetch('/api/health').then(r=>r.json()).then(x=>{if(!x.eodhdConfigured){const b=$('#setupBanner');b.classList.remove('hidden');b.innerHTML='<b>Depot-Cockpit Professional 3.2.1 ist aktiv.</b><br>Für EODHD-Marktdaten fehlt noch der geschützte Schlüssel in Vercel.'}else{refresh()}}).catch(()=>{});
