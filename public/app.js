...
const APP_VERSION='4.3 Diagnose';
...
function diagEvent(type,detail={}){
  diagnosticRuntime.events.push({at:new Date().toISOString(),type,...detail});
  if(diagnosticRuntime.events.length>60)diagnosticRuntime.events.shift();
}
diagEvent('app-start',{host:location.hostname,href:location.href,release:APP_VERSION});
...
function saveMarketCache(){
  diagnosticRuntime.saveCalls++;
  const attempt={at:new Date().toISOString(),key:MARKET_CACHE};
  try{
    const cleaned=sanitizeMarketData(state.data);
    attempt.runtimeEntries=Object.keys(state.data||{}).length;
    attempt.validEntries=Object.keys(cleaned).length;
    if(!attempt.validEntries){
      attempt.ok=false;
      attempt.reason='Keine gültigen Marktdaten im Laufzeitspeicher';
      diagnosticRuntime.lastSave=attempt;
      diagEvent('cache-save-skipped',attempt);
      return false
    }
    ...
    localStorage.setItem(MARKET_CACHE,raw);
    const verify=localStorage.getItem(MARKET_CACHE);
    attempt.ok=verify===raw;
    attempt.bytes=raw.length;
    attempt.savedAt=marketCacheSavedAt;
    attempt.reason=attempt.ok?'Gespeichert und zurückgelesen':'Rückleseprüfung fehlgeschlagen';
    diagnosticRuntime.lastSave=attempt;
    diagEvent(attempt.ok?'cache-save-ok':'cache-save-failed',attempt);
    return attempt.ok
  }catch(error){
    attempt.ok=false;
    attempt.reason=error?.message||String(error);
    diagnosticRuntime.lastSave=attempt;
    diagEvent('cache-save-error',attempt);
    return false
  }
}
...
function loadMarketCache(){
  diagnosticRuntime.loadCalls++;
  const attempt={at:new Date().toISOString(),keys:[]};
  const keys=[MARKET_CACHE,...LEGACY_MARKET_CACHES];
  const candidates=[];
  for(const key of keys){
    const raw=localStorage.getItem(key);
    const info={key,exists:Boolean(raw),bytes:raw?.length||0};
    if(raw){
      try{
        const parsed=JSON.parse(raw);
        const cleaned=sanitizeMarketData(parsed?.data);
        ...
        info.savedAt=parsed?.savedAt||null;
        info.updatedAt=parsed?.updatedAt||null;
        info.rawEntries=parsed?.data&&typeof parsed.data==='object'?Object.keys(parsed.data).length:0;
        info.validEntries=Object.keys(cleaned).length;
        const savedAt=parsed?.savedAt||parsed?.updatedAt||null;
        info.ageMs=savedAt?Date.now()-new Date(savedAt).getTime():null;
        info.accepted=Boolean(savedAt)&&Number.isFinite(info.ageMs)&&info.ageMs<=MARKET_MAX_AGE_MS&&info.validEntries>0;
        if(!info.accepted){
          info.rejectReason=!savedAt?'Zeitstempel fehlt':!Number.isFinite(info.ageMs)?'Zeitstempel ungültig':info.ageMs>MARKET_MAX_AGE_MS?'älter als 14 Tage':'keine gültigen Kurse';
        }else{
          candidates.push({key,savedAt,updatedAt:parsed.updatedAt||savedAt,data:cleaned})
        }
      }catch(error){
        info.parseOk=false;
        info.rejectReason=`JSON-Fehler: ${error?.message||error}`;
      }
    }
    attempt.keys.push(info)
  }

  const best=candidates.sort((a,b)=>new Date(b.savedAt)-new Date(a.savedAt))[0];
  if(!best){
    attempt.ok=false;
    attempt.reason='Kein akzeptierter Cache gefunden';
    diagnosticRuntime.lastLoad=attempt;
    diagEvent('cache-load-empty',{keys:attempt.keys});
    return false
  }
  ...
  attempt.ok=true;
  attempt.selectedKey=best.key;
  attempt.validEntries=Object.keys(best.data).length;
  attempt.savedAt=best.savedAt;
  ...
  try{
    ...
    attempt.migrationWrite=true
  }catch(error){
    attempt.migrationWrite=false;
    attempt.migrationError=error?.message||String(error)
  }
  diagnosticRuntime.lastLoad=attempt;
  diagEvent('cache-load-ok',attempt);
  return true
}
...
function inspectLocalStorage(){
  ...
  try{
    for(let i=0;i<localStorage.length;i++){
      ...
      if(!key||!/(th66|depot|market|professional)/i.test(key))continue;
      const raw=localStorage.getItem(key)||'';
      const row={key,bytes:raw.length,parseOk:false};
      try{
        const parsed=JSON.parse(raw);
        row.parseOk=true;
        row.schemaVersion=parsed?.schemaVersion??null;
        row.savedAt=parsed?.savedAt||null;
        row.updatedAt=parsed?.updatedAt||null;
        const data=parsed?.data;
        row.entries=data&&typeof data==='object'?Object.keys(data).length:0;
        row.validEntries=Object.keys(sanitizeMarketData(data)).length;
      }catch(error){
    ...
    rows.push({key:'localStorage',error:error?.message||String(error)})
  }
...
function diagnosticReport(){
  return {
    ...
    location:{host:location.hostname,href:location.href,protocol:location.protocol},
    storage:{
      mainKey:STORE,
      marketKey:MARKET_CACHE,
      legacyMarketKeys:LEGACY_MARKET_CACHES,
      available:(()=>{try{const k='__th66_test__';localStorage.setItem(k,'1');localStorage.removeItem(k);return true}catch{return false}})(),
      keys:inspectLocalStorage()
    },
    runtime:{
      positions:state.positions.length,
      dataEntries:Object.keys(state.data||{}).length,
      validMarketEntries:Object.keys(sanitizeMarketData(state.data)).length,
      updatedAt:state.updatedAt,
      marketCacheSavedAt,
      cacheAgeMinutes:Number.isFinite(marketCacheAge())?Math.round(marketCacheAge()/60000):null
    },
...
function reloadCacheForDiagnosis(){
  const before=Object.keys(sanitizeMarketData(state.data)).length;
  const ok=loadMarketCache();
  render();
  toast(ok?`Cache geladen · ${Object.keys(sanitizeMarketData(state.data)).length} gültige Werte`:`Kein gültiger Cache · vorher ${before}`)
}

function renderDiagnostics(){
  const box=$('#diagnosticsContent');if(!box)return;
  const report=diagnosticReport();
  const keys=report.storage.keys;
  const keyRows=keys.length?keys.map(k=>`<tr>
    <td>${k.key}</td>
    <td>${k.bytes??0}</td>
    <td>${k.parseOk?'Ja':'Nein'}</td>
    <td>${k.schemaVersion??'–'}</td>
    <td>${k.entries??'–'}</td>
    <td>${k.validEntries??'–'}</td>
    <td>${k.savedAt?new Date(k.savedAt).toLocaleString('de-DE'):'–'}</td>
    <td>${k.error||''}</td>
  </tr>`).join(''):'<tr><td colspan="8">Keine passenden lokalen Speicher gefunden.</td></tr>';

  const load=diagnosticRuntime.lastLoad;
  const save=diagnosticRuntime.lastSave;
  box.innerHTML=`<div class="diagnostic-alert">
    <b>Diagnosemodus – Release 4.3</b>
    <span>Diese Ansicht misst Speicherung und Laden. Sie verändert keine Depotdaten.</span>
  </div>
  <div class="diagnostic-grid">
    <span>Adresse<b>${location.hostname}</b></span>
    <span>Hauptspeicher<b>${STORE}</b></span>
    <span>Marktdaten-Speicher<b>${MARKET_CACHE}</b></span>
    <span>localStorage verfügbar<b>${report.storage.available?'Ja':'Nein'}</b></span>
    <span>Positionen<b>${report.runtime.positions}</b></span>
    <span>Laufzeit-Datensätze<b>${report.runtime.dataEntries}</b></span>
    <span>Davon gültige Kurse<b>${report.runtime.validMarketEntries}</b></span>
    <span>Cache-Alter<b>${report.runtime.cacheAgeMinutes==null?'nicht vorhanden':report.runtime.cacheAgeMinutes+' Minuten'}</b></span>
    <span>Letzter Ladeversuch<b>${load?(load.ok?'Erfolgreich':'Fehlgeschlagen'):'noch keiner'}</b></span>
    <span>Gewählter Cache<b>${load?.selectedKey||'–'}</b></span>
    <span>Letzter Speicherversuch<b>${save?(save.ok?'Erfolgreich':'Fehlgeschlagen'):'noch keiner'}</b></span>
    <span>Speichergrund<b>${save?.reason||load?.reason||'–'}</b></span>
  </div>
  <div class="diagnostic-actions">
    <button class="secondary" id="copyDiagnosticBtn">Diagnose kopieren</button>
    <button class="secondary" id="reloadDiagnosticCacheBtn">Cache erneut prüfen</button>
  </div>
  <div class="diagnostic-table-wrap"><table class="diagnostic-table">
    <thead><tr><th>Schlüssel</th><th>Bytes</th><th>JSON</th><th>Schema</th><th>Einträge</th><th>Gültig</th><th>Gespeichert</th><th>Fehler</th></tr></thead>
    <tbody>${keyRows}</tbody>
  </table></div>
  <details class="diagnostic-log"><summary>Technisches Ereignisprotokoll (${diagnosticRuntime.events.length})</summary>
    <pre>${JSON.stringify(diagnosticRuntime.events,null,2)}</pre>
  </details>`;
  $('#copyDiagnosticBtn').onclick=copyDiagnosticReport;
  $('#reloadDiagnosticCacheBtn').onclick=reloadCacheForDiagnosis;
}
...
async function refresh({force=false,quiet=false}={}){
  ...
  try{
    ...
    diagnosticRuntime.lastApi={
      at:new Date().toISOString(),
      httpStatus:r.status,
      ok:Boolean(x?.ok),
      rateLimited:Boolean(x?.rateLimited),
      networkRequests:x?.networkRequests??null,
      cacheHits:x?.cacheHits??null,
      resultCount:Array.isArray(x?.results)?x.results.length:0,
      successfulResults:Array.isArray(x?.results)?x.results.filter(v=>v?.ok).length:0,
      error:x?.error||null
    };
    diagEvent('api-response',diagnosticRuntime.lastApi);
    ...
    if(successCount>0){
      ...
      diagEvent('api-merge-success',{successCount,totalResults:Array.isArray(x.results)?x.results.length:0});
      saveMarketCache()
    }else if(hadGoodData){
