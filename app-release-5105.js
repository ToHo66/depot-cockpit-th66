/* Depot-Cockpit Professional 5.10.5
...
(() => {
  ...
  const BUILD_VERSION = '5.10.5';
  const BUILD_STAMP = 'BUILD CRYPTO-ROUTING-FIX · 2026-08-14 · 15:20';
    ...
    runSystemDiagnosis = async function() {
      ...
      try {
        ...
        showDiagnostic('Systemprüfung abgeschlossen', [
          ...
          'Aktive UI-Architektur: app.js + app-release-5105.js · ein konsolidierter Daten-/UI-Layer',
...
/* Depot-Cockpit 5.10.5 – DATA CORE V3
...
(() => {
  ...
  function payload590() {
    return state.positions.filter(p => !['CRYPTO_DIRECT','CRYPTO_YAHOO'].includes(p.dataSource)).map(p => ({
      id:p.id,
      name:p.name,
      isin:p.isin || '',
      wkn:p.wkn || '',
      ...
      candidates:(() => {
        ...
        if (!base || p.id === 'trilogy' || ['CRYPTO_DIRECT','CRYPTO_YAHOO'].includes(p.dataSource)) return [];
  ...
  async function refresh590() {
    ...
    showDiagnostic('Datenkern 5.10.5 prüft alle Depotpositionen', [
    ...
    try {
      ...
      // Crypto is deliberately isolated from the securities data core.
      // A failure here can never invalidate the other depot quotes.
      let cryptoResult = null;
      let cryptoDiagnostic = null;
      const ethereum = state.positions.find(p => p.id === 'crypto-ethereum-eth');
      if (ethereum) {
        try {
          const cryptoCall = await fetchJsonWithTimeout('/api/crypto-quote-v1', {}, 12000);
          const cb = cryptoCall.body;
          if (cryptoCall.response.ok && cb?.ok && Number.isFinite(Number(cb.price))) {
            cryptoResult = {
              id:ethereum.id, ok:true,
              latest:{price:Number(cb.price), date:String(cb.asOf || new Date().toISOString()).slice(0,10)},
              source:cb.source || 'ETH/EUR', usedVenue:'Crypto', currency:'EUR',
              sourceMeta:{provider:cb.provider || 'CRYPTO_DIRECT', sourceKind:cb.source || 'ETH/EUR · 24/7', delayedMinutes:0, asOf:cb.asOf || null}
            };
            cryptoDiagnostic = {id:ethereum.id,name:'Ethereum',status:'FOUND',statusLabel:'ETH/EUR direkt',source:cryptoResult.source};
          } else {
            cryptoDiagnostic = {id:ethereum.id,name:'Ethereum',status:'NOT_FOUND',statusLabel:cb?.error || `Crypto HTTP ${cryptoCall.response.status}`,source:null};
          }
        } catch (e) {
          cryptoDiagnostic = {id:ethereum.id,name:'Ethereum',status:'NOT_FOUND',statusLabel:e?.message || String(e),source:null};
        }
      }

      const combinedResults = [...(body.results || []).filter(x => x?.ok), ...(cryptoResult ? [cryptoResult] : [])];
      const combinedDiagnostics = [...(body.positionDiagnostics || []), ...(cryptoDiagnostic ? [cryptoDiagnostic] : [])];
      const snapshot = {
        ok:true,
        provider:'Depot-Cockpit Market Data V3 + Crypto Direct V1',
        generatedAt:body.generatedAt || new Date().toISOString(),
        requestedIds:state.positions.map(p => p.id),
        results:combinedResults,
        diagnostics:{...(body.diagnostics || {}), cryptoFound:Boolean(cryptoResult)}
      };
      ...
      const report = combinedDiagnostics;
      const lines = [
        `Speicher: ${mode}`,
        `Neue Treffer dieses Laufs: ${combinedResults.length}/${state.positions.length}`,
      ...
      showDiagnostic(
        combinedResults.length === state.positions.length ? 'Datenkern 5.10.5: vollständige Versorgung' : 'Datenkern 5.10.5: Prüfung abgeschlossen',
        lines,
        combinedResults.length === state.positions.length ? 'success' : 'info'
      );
      toast(`${Object.keys(stored.items || {}).length}/${state.positions.length} Positionen verwertbar`);
    } catch (error) {
      showDiagnostic('Datenkern 5.10.5 fehlgeschlagen', [
...
(() => {
  ...
  const VERSION_595 = '5.10.5';
  const BUILD_595 = 'BUILD CRYPTO-ROUTING-FIX · 2026-08-14 · 15:20';
