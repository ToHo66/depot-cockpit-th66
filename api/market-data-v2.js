...
const KEY='__TH66_DB_DELAYED_SNAPSHOT_540__';
...
function fresh(positions){if(!(cache.savedAt&&Date.now()-cache.savedAt<TTL_MS))return false;const want=new Set(positions.map(p=>p.id));const have=Object.keys(cache.items||{});return have.length>0&&have.every(id=>want.has(id))&&cache.diagnostics?.requested===positions.length}
...
export default async function handler(req,res){if(req.method!=='POST')return send(res,405,{ok:false,error:'Nur POST erlaubt.'});const positions=Array.isArray(req.body?.positions)?req.body.positions:[];if(!positions.length)return send(res,400,{ok:false,error:'Keine Positionen übergeben.'});if(fresh(positions)){const results=positions.map(p=>cache.items[p.id]).filter(Boolean);return send(res,200,{ok:true,provider:'Deutsche Börse / Xetra Delayed Post-Trade',delayedMinutes:15,generatedAt:new Date(cache.savedAt).toISOString(),fromCache:true,sourceFiles:cache.sourceFiles,results,diagnostics:{...cache.diagnostics,requested:positions.length,found:results.length}})}const s=await snapshot(positions);if(s.results.length){cache.savedAt=Date.now();cache.items=Object.fromEntries(s.results.map(x=>[x.id,x]));cache.sourceFiles=s.used.slice(0,8);cache.diagnostics=s.diagnostics}return send(res,200,{ok:true,provider:'Deutsche Börse / Xetra Delayed Post-Trade',delayedMinutes:15,generatedAt:new Date().toISOString(),fromCache:false,sourceFiles:s.used.slice(0,8),results:s.results,diagnostics:s.diagnostics,warning:s.results.length===positions.length?null:`${positions.length-s.results.length} Instrument(e) hatten im durchsuchten Xetra-Handelstag keinen eindeutig zuordenbaren Post-Trade-Treffer.`})}
