...
(() => {
  ...
  function normalizeEthereum() {
    ...
    if(changed){
      writeBackup(raw,'pre-5.10.2-ethereum-normalize');
      snap.savedAt=new Date().toISOString();
      snap.normalized5102Ethereum={at:snap.savedAt,changes:[...new Set(changes)]};
  ...
  function normalizeSpaceX() {
    ...
    if(changed){
      writeBackup(raw,'pre-5.10.2-spacex-normalize');
      snap.savedAt=new Date().toISOString();
      snap.normalized5102SpaceX={at:snap.savedAt, isin:SPACEX.isin, changes:[...new Set(changes)]};
      nativeSetItem.call(localStorage, STORE, JSON.stringify(snap));
    }
  ...
  window.__DC_PRELOAD_5102__={version:VERSION,store:STORE,backup:BACKUP,history:HISTORY,spaceX:spaceXResult,ethereum:ethereumResult};
  console.info('Depot-Cockpit 5.10.2 crypto preload active', window.__DC_PRELOAD_5102__);
})();
