/* Depot-Cockpit Professional 5.10.5
   ...
   4) Add the verified Ethereum Trade Republic position idempotently as its own Crypto type.
   5) Never change unrelated instruments merely because SpaceX or Ethereum needs normalization.
*/
(() => {
  'use strict';
  ...
  const ETHEREUM = Object.freeze({
    id:'crypto-ethereum-eth',
    name:'Ethereum',
    isin:'',
    wkn:'',
    qty:3.363942,
    broker:'Trade Republic',
    brokerDisplaySource:'Trade Republic Crypto',
    brokerVenue:'Crypto',
    analysisVenue:'Crypto',
    analysisExchangeCode:'CRYPTO',
    exchangeCode:'CRYPTO',
    fallbackVenues:[],
    dataSource:'CRYPTO_DIRECT',
    analysisSymbol:'ETH-EUR',
    marketSymbol:'ETH-EUR',
    currency:'EUR',
    purchasePrice:3567.243430475317,
  ...
  function isEthereum(x) {
    return String(x?.id || '') === ETHEREUM.id ||
      String(x?.analysisSymbol || x?.marketSymbol || '').toUpperCase() === 'ETH-EUR' ||
      String(x?.name || '').trim().toLowerCase() === 'ethereum';
  }
  function normalizeEthereum() {
    const raw=localStorage.getItem(STORE), snap=parse(raw); if(!snap)return {changed:false,reason:'no-store'};
    if((snap.archive||[]).some(isEthereum)) return {changed:false,reason:'archived'};
    let p=(snap.positions||[]).find(isEthereum), changed=false, changes=[];
    if(!p){
      p=clone(ETHEREUM); snap.positions.push(p); changed=true; changes.push('add');
    }
    const route={
      id:ETHEREUM.id,name:ETHEREUM.name,qty:ETHEREUM.qty,broker:ETHEREUM.broker,
      brokerDisplaySource:ETHEREUM.brokerDisplaySource,brokerVenue:ETHEREUM.brokerVenue,
      analysisVenue:ETHEREUM.analysisVenue,analysisExchangeCode:ETHEREUM.analysisExchangeCode,
      exchangeCode:ETHEREUM.exchangeCode,fallbackVenues:[],dataSource:ETHEREUM.dataSource,
      analysisSymbol:ETHEREUM.analysisSymbol,marketSymbol:ETHEREUM.marketSymbol,currency:'EUR',
      purchasePrice:ETHEREUM.purchasePrice,purchaseAmount:ETHEREUM.purchaseAmount,
      assetType:'Crypto',type:'Crypto'
    };
    for(const [k,v] of Object.entries(route)){
      if(JSON.stringify(p[k])!==JSON.stringify(v)){p[k]=v;changed=true;changes.push(k);}
    }
    if(changed){
      writeBackup(raw,'pre-5.10.5-ethereum-normalize');
      snap.savedAt=new Date().toISOString();
      snap.normalized5105Ethereum={at:snap.savedAt,changes:[...new Set(changes)]};
      nativeSetItem.call(localStorage,STORE,JSON.stringify(snap));
    }
    return {changed,qty:Number(p.qty),purchaseAmount:Number(p.purchaseAmount),changes:[...new Set(changes)]};
  }
  function normalizeSpaceX() {
    ...
    if(changed){
      writeBackup(raw,'pre-5.10.5-spacex-normalize');
      snap.savedAt=new Date().toISOString();
      snap.normalized5105SpaceX={at:snap.savedAt, isin:SPACEX.isin, changes:[...new Set(changes)]};
      nativeSetItem.call(localStorage, STORE, JSON.stringify(snap));
    }
  ...
  const ethereumResult = normalizeEthereum();
  ...
  window.__DC_PRELOAD_5105__={version:VERSION,store:STORE,backup:BACKUP,history:HISTORY,spaceX:spaceXResult,ethereum:ethereumResult};
  console.info('Depot-Cockpit 5.10.5 crypto preload active', window.__DC_PRELOAD_5105__);
})();
