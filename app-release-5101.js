/* Depot-Cockpit Professional 5.10.1
...
(() => {
  ...
  const BUILD_VERSION = '5.10.1';
  const BUILD_STAMP = 'BUILD CLEAN-CONSOLIDATED · 2026-08-14 · 12:28';
...
/* Depot-Cockpit 5.10.1 – DATA CORE V3
...
(() => {
  ...
  async function refresh590() {
    ...
    showDiagnostic('Datenkern 5.10.1 prüft alle Depotpositionen', [
    ...
    try {
      ...
      showDiagnostic(
        body.complete ? 'Datenkern 5.10.1: vollständige Versorgung' : 'Datenkern 5.10.1: Prüfung abgeschlossen',
        lines,
        body.complete ? 'success' : 'info'
      );
      toast(`${Object.keys(stored.items || {}).length}/${state.positions.length} Positionen verwertbar`);
    } catch (error) {
      showDiagnostic('Datenkern 5.10.1 fehlgeschlagen', [
...
(() => {
  ...
  const VERSION_595 = '5.10.1';
  const BUILD_595 = 'BUILD CLEAN-CONSOLIDATED · 2026-08-14 · 12:28';
