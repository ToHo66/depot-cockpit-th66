...
(() => {
  ...
  const BUILD_VERSION = '5.10.3';
  const BUILD_STAMP = 'BUILD ETHEREUM-QUOTE-FIX · 2026-08-14 · 13:47';
    ...
    runSystemDiagnosis = async function() {
      ...
      try {
        ...
        showDiagnostic('Systemprüfung abgeschlossen', [
          ...
          'Aktive UI-Architektur: app.js + app-release-5103.js · ein konsolidierter Daten-/UI-Layer',
...
/* Depot-Cockpit 5.10.3 – DATA CORE V3
...
(() => {
  ...
  async function refresh590() {
    ...
    showDiagnostic('Datenkern 5.10.3 prüft alle Depotpositionen', [
    ...
    try {
      ...
      showDiagnostic(
        body.complete ? 'Datenkern 5.10.3: vollständige Versorgung' : 'Datenkern 5.10.3: Prüfung abgeschlossen',
        lines,
        body.complete ? 'success' : 'info'
      );
      toast(`${Object.keys(stored.items || {}).length}/${state.positions.length} Positionen verwertbar`);
    } catch (error) {
      showDiagnostic('Datenkern 5.10.3 fehlgeschlagen', [
...
(() => {
  ...
  const VERSION_595 = '5.10.3';
  const BUILD_595 = 'BUILD ETHEREUM-QUOTE-FIX · 2026-08-14 · 13:47';
